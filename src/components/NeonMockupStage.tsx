...
const NeonMockupStage: React.FC<NeonMockupStageProps> = ({
  ...
  const S: Record<string, React.CSSProperties> = {
    ...
    plane:{
      ...
      transform:`translate(-50%,-70%) translate(${drag.dx.toFixed(2)}px, ${drag.dy.toFixed(2)}px) scale(1)`,
  ...
  useEffect(()=>{ // Drag → Transform anpassen
    planeRef.current && (planeRef.current.style.transform =
      `translate(-50%,-70%) translate(${drag.dx.toFixed(2)}px, ${drag.dy.toFixed(2)}px) scale(1)`);
  }, [drag.dx, drag.dy]);
